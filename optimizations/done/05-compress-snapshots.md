...
zstd level 1 on `Snapshot` (BNSS) and `ExitDelta` (BNDL) files. Read
path sniffs the zstd frame magic (`28 B5 2F FD`) so legacy
uncompressed fixtures still load. `load_savestate_file` was updated to
decompress before its BNSS prefix check.

`make verify` (bk2 harness, 4154 pairs):

| State | Wall | Fresh entry size (sample) | Session dir |
|---|---|---|---|
| Pre-05 (10A in place) | 1:49.98 | 692 KB | ~2-3 GB est. |
| Post-05 | 1:53.07 (+1.8%) | **42 KB (-94%)** | 492 MB (bk2/) |

Disk dropped ~13× on the canonical bk2 workload (256 KB EWRAM with
huge zero runs compresses brilliantly). Wall-time slightly worse on
this SSD because dedup already meant disk wasn't bottlenecking — the
compression CPU cost edged out the I/O savings. Still net-positive:
fixtures are tractable to keep around / inspect / archive.
